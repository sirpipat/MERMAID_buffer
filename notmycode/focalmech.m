...
colormap(ax, [1 1 1; fillcolor])    % 2 color beachball fill 
...
if eig1 <= 0     % implosion
    fill(ax, centerX+u,centerY+w,'w')   % fill white circle
    return
end
if eig3 >= 0     % explosion
    fill(ax, centerX+u,centerY+w,fillcolor)   % filled circle
    return
end
...
hold on
if ~colorit
    plot(ax, centerX+u,centerY+w,'k')   % plot circle boundary
    c = contourc(ax, centerX+sE*y,centerY+sN*x,uz,[0 0]);       % no fill
    i = c(2,:) >= 1;
    c(:,i) = nan;
    plot(ax, centerX+sE*c(1,:),centerY+sN*c(2,:),'color',fillcolor)   % plot circle boundary    
else
    fill(ax, centerX+u,centerY+w,'w')   % fill white circle
    plot(ax, centerX+u,centerY+w,'k')   % plot circle boundary
...
if nc == 1       % 1 contour (closed unless single plane)
    ...
    if d2 > 2 % single plane
        ...
        ac = [cos(a1) sin(a1)];
        c1 = [c; ac];
        fill(ax, centerX+sE*c1(:,1),centerY+sN*c1(:,2),fillcolor)  
        ...
        if uv > 0   % fill inside contour
            fill(ax, centerX+sE*c(:,1),centerY+sN*c(:,2),fillcolor)
        elseif uv < 0   % fill outside contour
            fill(ax, centerX+u,centerY+w,fillcolor)
            fill(ax, centerX+sE*c(:,1),centerY+sN*c(:,2),'w') 
        end
    end
else                    % 2 contours
    ...
    c1 = [c(2:ia(2),:); ac; c(2,:)];
    if uvc < 0
        fill(ax, centerX+u,centerY+w,fillcolor)   % fill colored circle
        fill(ax, centerX+sE*c1(:,1),centerY+sN*c1(:,2),[ 1 1 1]) 
    else
        fill(ax, centerX+sE*c1(:,1),centerY+sN*c1(:,2),fillcolor)
    end
    ...
    if uvc < 0
        fill(ax, centerX+sE*c2(:,1),centerY+sN*c2(:,2),'w')        
    else
        fill(ax, centerX+sE*c2(:,1),centerY+sN*c2(:,2),fillcolor)
    end        
end
end
...
if false
    if vT(3) >= 0
        text(ax, centerX+sE*vT(2),centerY+sN*vT(1),'T','color',[0 1 0],'FontSize',12)
    else
        text(ax, centerX-sE*vT(2),centerY-sN*vT(1),'T','color',[0 1 0],'FontSize',12)
    end
    if vB(3) >= 0
        text(ax, centerX+sE*vB(2),centerY+sN*vB(1),'B','color',[0 0 1],'FontSize',12)
    else
        text(ax, centerX-sE*vB(2),centerY-sN*vB(1),'B','color',[0 0 1],'FontSize',12)
    end
    if vP(3) >= 0
        text(ax, centerX+sE*vP(2),centerY+sN*vP(1),'P','color',[1 0 0],'FontSize',12)
    else
        text(ax, centerX-sE*vP(2),centerY-sN*vP(1),'P','color',[1 0 0],'FontSize',12)
    end
end
...
if ~isempty(ctitle)
    text(ax, centerX,centerY+sN*1.25,ctitle,'HorizontalAlignment','center','FontSize',fsize)
end
end
